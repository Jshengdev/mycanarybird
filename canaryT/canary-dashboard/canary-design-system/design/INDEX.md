...
### "What logo or wordmark should I use?"
→ `tokens/logo.md`
...
17. **Section spacing** → `40px` between every section on every page. This applies between headers and content sections, between adjacent sections, and between the last section and any footer content. Use `marginBottom: '40px'` on section containers.
18. **Large numbers** → All large display numbers (scores, stat values, prices) use `fontWeight: 400` (regular). Never bold or semibold on numbers larger than 16px.
19. **Button text alignment** → Buttons with AsciiHover on one side (`asciiVariant: 'right'`) must have text and icon left-aligned (`justifyContent: 'flex-start'`). Only buttons with AsciiHover on both sides (`asciiVariant: 'both'`) are centered. Dropdown trigger buttons always have text+icon on the left and chevron pinned to the far right.
20. **Settings sidebar** → Settings pages have their own dedicated sidebar (not a panel inside the main sidebar). The main sidebar's Settings button navigates to `/settings/workspace`. The settings sidebar has a PanelLeft icon that opens the main sidebar as a slide-over overlay.
21. **Inline metadata separators** → When multiple metadata values appear on a single horizontal line (e.g. duration, events, date), separate them with ` · ` (middle dot). Never use pipes, dashes, or plain spaces as separators.
22. **Suggested rule formatting** → When displaying a rule's plain English description, bold the action word (Block, Flag, Observe), the verb (navigates, accesses, visits), and any quoted target strings. Use `<strong>` tags via dangerouslySetInnerHTML or equivalent.
...
│   └── logo.md                   ← logo SVG usage, sizing, color rules
├── components/
