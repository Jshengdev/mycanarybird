...
export function RulesetDetail({ rulesetId }: RulesetDetailProps) {
  ...
  // RULESET_RULES contains all rules — filter by ruleset if needed
  const rulesetRules = rulesetId === 'photon-research-rules' ? RULESET_RULES : RULESET_RULES.filter((r) => r.type === 'boundary' && r.severity === 'critical');

  const filteredRules = useMemo(() => {
    if (!search) return rulesetRules;
    const q = search.toLowerCase();
    return rulesetRules.filter((r) =>
  ...
  const evaluatedCount = rulesetRules.filter((r) => r.stats.totalEvaluations > 0).length;
  ...
  return (
    <div className="flex flex-col" style={{ padding: 'var(--space-8)', gap: 'var(--space-6)' }}>
      ...
      <RulesetStatCards sessions={SESSIONS} rules={rulesetRules} />
      <RulesetMetaBar rules={rulesetRules} onTabChange={(tab) => setSelectedTab(tab as TabId)} />
      ...
      {selectedTab === 'conflicts' && <ConflictsTab rules={rulesetRules} />}
      ...
      <div className="flex items-center justify-between" style={{ borderTop: '1px solid var(--grey-stroke)', paddingTop: 'var(--space-4)', marginTop: 'var(--space-4)' }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--icon-grey)' }}>
          {rulesetRules.length} rules · {evaluatedCount}/{rulesetRules.length} evaluated
        </span>
