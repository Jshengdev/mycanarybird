/**
 * AsciiHover.jsx
 * Canary Dashboard — ASCII block unfurl hover effect
 *
 * Usage:
 *   import { AsciiHover } from '@/components/ui/AsciiHover'
 *
 *   // Wrap any element — it inherits padding automatically
 *   <AsciiHover>
 *     <button className="btn">New Rule</button>
 *   </AsciiHover>
 *
 *   // Variants
 *   <AsciiHover variant="right">...</AsciiHover>   // default — blocks on right only
 *   <AsciiHover variant="both">...</AsciiHover>    // blocks on left + right
 *   <AsciiHover variant="left">...</AsciiHover>    // blocks on left only
 *
 *   // Custom color (defaults to #858585)
 *   <AsciiHover color="#d4a017">...</AsciiHover>
 *
 *   // Controlled externally (e.g. parent manages hover state)
 *   <AsciiHover active={isHovered}>...</AsciiHover>
 *
 * Notes:
 *   - Wraps children in a relative/overflow:hidden container
 *   - SVG blocks scale to fill parent height minus its computed padding
 *   - Safe to nest inside other hover effects — uses its own mouseenter/mouseleave
 *   - Fully composable: pass className/style to the wrapper as needed
 */

import { useRef, useEffect, useCallback, useState } from 'react'

// ─── SVG path data (your exact Figma exports, 12×28 viewBox) ─────────────────
...
// Sequence: right side renders lighter→dark (░▒▓█), dark is outermost
// Sequence: left side renders dark→lighter (█▓▒░), dark is outermost
const SEQUENCE = ['lighter', 'light', 'medium', 'dark'] // index 3 = outermost

// Opacity per block, outermost (dist=0) → innermost (dist=3)
const OPACITIES = [0.8, 0.6, 0.4, 0.2]
const STAGGER   = 35  // ms between each block
const DURATION  = 50  // ms per fade transition
...
function AsciiStrip({ side, size, opacities, color }) {
  // right: lighter(0) light(1) medium(2) dark(3) — dark flush to right edge
  // left:  dark(3) medium(2) light(1) lighter(0) — dark flush to left edge
...
function useAsciiAnimation() {
  const stateRef  = useRef('out')   // 'out' | 'animating-in' | 'in' | 'animating-out'
  const timersRef = useRef([])
  const queuedRef = useRef(null)    // 'in' | 'out' | null

  // opacities[dist] for each block — driven by state
  ...
  const setOpacity = useCallback((dist, value) => {
    // dist=0 outermost, applied to both sides symmetrically
  ...
  // Support controlled `active` prop
...
/**
 * @param {object}  props
 * @param {'right'|'left'|'both'} props.variant  - which sides to show blocks on
 * @param {string}  props.color                  - block fill color (hex/rgb/var)
 * @param {boolean} [props.active]               - controlled hover state (optional)
 * @param {string}  [props.className]            - applied to wrapper div
 * @param {object}  [props.style]                - applied to wrapper div
...
export function AsciiHover({
  ...
  // Measure height on mount + resize to size blocks correctly
  useEffect(() => {
  ...
  // Controlled mode: respond to `active` prop changes
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* ASCII strips — z-index 0, behind content */}
      {showRight && (
        <AsciiStrip
          side="right"
          size={blockSize}
          opacities={opacitiesR}
          color={color}
        />
      )}
      {showLeft && (
        <AsciiStrip
          side="left"
          size={blockSize}
          opacities={opacitiesL}
          color={color}
        />
      )}

      {/* Content — z-index 1, always on top */}
...
// ─── Named export of the hook for advanced use ────────────────────────────────
// Use when you need to sync ascii state with other hover effects on the same element
//
// Example:
//   const { opacitiesR, opacitiesL, handleEnter, handleLeave } = useAsciiAnimation()
//   <div onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
//     <AsciiStrip side="right" size={20} opacities={opacitiesR} color="#858585" />
//     <AsciiStrip side="left"  size={20} opacities={opacitiesL} color="#858585" />
//     <span>content</span>
//   </div>
