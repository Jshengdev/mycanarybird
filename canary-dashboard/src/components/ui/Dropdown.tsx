...
function DropdownItem({ selected, label, onClick }: { selected: boolean; label: string; onClick: () => void }) {
  ...
  return (
    <div className="flex flex-col" style={{ gap: 'var(--space-1)', position: 'relative' }} ref={ref}>
      ...
      <button
        ...
        onMouseEnter={(e) => {
          if (!open) e.currentTarget.style.background = 'var(--hover-gray)';
        }}
        onMouseLeave={(e) => {
          if (!open) e.currentTarget.style.background = 'var(--card-bg)';
        }}
      >
        ...
        >
          {groupKeys.map((gk, gi) => (
            <React.Fragment key={gk}>
              {gi > 0 && <div style={{ height: '1px', background: 'var(--grey-stroke)', margin: '2px 0' }} />}
              {groups[gk].map((opt) => (
                <DropdownItem key={opt.value} selected={opt.value === value} label={opt.label} onClick={() => { onChange?.(opt.value); setOpen(false); }} />
              ))}
            </React.Fragment>
          ))}
        </div>
