...
export function AgentHealthCards() {
  ...
  const handleCardMove = useCallback((id: string, e: React.MouseEvent<HTMLDivElement>) => {
    const rect = e.currentTarget.getBoundingClientRect();
    const x = ((e.clientX - rect.left) / rect.width) * 100;
    const y = ((e.clientY - rect.top) / rect.height) * 100;
    setBgStyles((prev) => ({ ...prev, [id]: `radial-gradient(circle at ${x}% ${y}%, #EDEDF5 0%, #F7F7FA 55%, #F5F5F8 100%)` }));
  }, []);
    ...
    >
      {AGENTS.map((agent, i) => {
        ...
        return (
          ...
          >
            <div
              onClick={() => router.push(`/${ws}/${agent.id}`)}
              onMouseEnter={() => setHoveredId(agent.id)}
              onMouseMove={(e) => handleCardMove(agent.id, e)}
              ...
              style={{
                ...
                padding: 'var(--space-8)',
                cursor: 'pointer',
                transition: hoveredId === agent.id ? 'none' : 'background 400ms ease',
                position: 'relative',
              }}
            >
              {/* LIVE chip */}
