...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--icon-grey)', letterSpacing: '0.06em', display: 'block', marginBottom: 'var(--space-4)' }}>
      {children}
    </span>
  );
}
    ...
    <div className="flex flex-col" style={{ gap: 'var(--space-8)' }}>
        ...
        {regenConfirm && (
          <div className="flex flex-col" style={{ gap: 'var(--space-3)', marginTop: 'var(--space-4)' }}>
            ...
            <div className="flex items-center" style={{ gap: 'var(--space-3)' }}>
              ...
              <span onClick={() => { setRegenConfirm(false); setRegenInput(''); }} style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', color: 'var(--accent-color)', cursor: 'pointer', textDecoration: 'underline' }}>Cancel</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-4)', marginTop: 'var(--space-4)' }}>
          {[{ label: 'AGENTS', value: '4 / 10' }, { label: 'SESSIONS THIS MONTH', value: '127 / Unlimited' }, { label: 'SESSION RETENTION', value: '90 days' }].map((item) => (
            <div key={item.label} className="flex flex-col items-center" style={{ background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', border: '1px solid var(--grey-stroke)', borderRadius: '4px', padding: 'var(--space-4)', gap: 'var(--space-2)' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', textTransform: 'uppercase', color: 'var(--icon-grey)' }}>{item.label}</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '18px', fontWeight: 600, color: 'var(--text-black)' }}>{item.value}</span>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <span style={{ fontFamily: 'var(--font-sans)', fontSize: '13px', color: 'var(--text-black)' }}>
          Team · <span style={{ fontFamily: 'var(--font-mono)' }}>$499</span> / month · Next billing <span style={{ fontFamily: 'var(--font-mono)' }}>Apr 30, 2026</span>{' '}
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-4)', marginBottom: 'var(--space-4)' }}>
          {SCORING_TEMPLATES.map((t) => (
            <div key={t.id} onClick={() => handleTemplateClick(t)} className={activeTemplate === t.id ? 'card-gradient-border' : ''} style={{ background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', border: activeTemplate === t.id ? 'none' : '1px solid var(--grey-stroke)', borderRadius: '4px', padding: 'var(--space-5)', cursor: 'pointer', position: 'relative' }}>
        ...
        {savingTemplate ? (
          <div className="flex items-center" style={{ gap: 'var(--space-3)' }}>
            ...
            <span onClick={() => setSavingTemplate(false)} style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', color: 'var(--accent-color)', cursor: 'pointer', textDecoration: 'underline' }}>Cancel</span>
          </div>
        ) : (
          <span onClick={() => setSavingTemplate(true)} style={{ fontFamily: 'var(--font-sans)', fontSize: '13px', color: 'var(--accent-color)', cursor: 'pointer' }}>+ Save current as template</span>
        )}
      </div>
...
function MembersSection() {
  ...
  return (
    <div className="flex flex-col" style={{ gap: 'var(--space-6)' }}>
      ...
      {/* Member row */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 80px', alignItems: 'center', padding: 'var(--space-4)', border: '1px solid var(--grey-stroke)', background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', borderRadius: '4px' }}>
        <div className="flex items-center" style={{ gap: 'var(--space-3)' }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-4)' }}>
      {integrations.map((int) => (
        <div key={int.name} className="flex flex-col" style={{ background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', border: '1px solid var(--grey-stroke)', borderRadius: '4px', padding: 'var(--space-6)', gap: 'var(--space-3)' }}>
  ...
  return (
    <div className="flex flex-col" style={{ gap: 'var(--space-8)' }}>
      ...
      <div>
        ...
        {AGENTS.map((agent) => (
          <div key={agent.id} className="flex items-center justify-between" style={{ padding: 'var(--space-3) var(--space-4)', background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', border: '1px solid var(--grey-stroke)', borderRadius: '4px', marginBottom: 'var(--space-2)' }}>
...
function WorkspaceActionsSection() {
  ...
  return (
    <div className="flex flex-col" style={{ gap: 'var(--space-4)' }}>
      {actions.map((action) => (
        <div key={action.name}>
          <div className="flex items-center justify-between" style={{ background: 'linear-gradient(160deg, #FDFDFD 0%, #F7F7FA 100%)', border: '1px solid var(--grey-stroke)', borderRadius: '4px', padding: 'var(--space-5)' }}>
          ...
          {action.confirm && (
            <div className="flex flex-col" style={{ gap: 'var(--space-3)', padding: 'var(--space-4)', border: '1px solid var(--grey-stroke)', borderTop: 'none' }}>
              ...
              <div className="flex items-center" style={{ gap: 'var(--space-3)' }}>
                ...
                <span onClick={() => { action.setConfirm(false); action.setInput(''); }} style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', color: 'var(--accent-color)', cursor: 'pointer', textDecoration: 'underline' }}>Cancel</span>
              </div>
            </div>
          )}
        </div>
...
export default function SettingsPage() {
  ...
  return (
    <div className="flex" style={{ minHeight: '100vh' }}>
      {/* Main sidebar — slides over settings sidebar when open */}
      <div
        style={{
          position: 'fixed',
          left: mainSidebarOpen ? '0' : '-280px',
          top: 0,
          zIndex: 50,
          transition: 'left 200ms ease',
        }}
      >
        <Sidebar activeItem={`settings-${section}`} onCollapse={() => setMainSidebarOpen(false)} />
      </div>
      {mainSidebarOpen && (
        <div
          onClick={() => setMainSidebarOpen(false)}
          style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,10,0.15)', zIndex: 40 }}
        />
      )}
          ...
          return (
            <div
              ...
              style={{
                ...
                paddingRight: '4px',
                cursor: 'pointer',
                background: isActive || isHovered ? 'var(--hover-gray)' : 'transparent',
                border: isActive || isHovered ? '1px solid var(--grey-stroke)' : '1px solid transparent',
                borderRadius: '0px',
                transition: 'background var(--transition-fast), border-color var(--transition-fast)',
                marginBottom: '2px',
              }}
            >
              <Icon size={12} style={{ color: 'var(--icon-grey)', flexShrink: 0 }} />
              <span style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', lineHeight: '16px', fontWeight: 500, color: 'var(--text-black)', flex: 1 }}>
                {s.label}
              </span>
            </div>
          );
